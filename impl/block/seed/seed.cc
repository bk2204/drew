...
static int seedtest(void *, const drew_loader_t *)
{
	using namespace drew;

	int res = 0;

	res |= BlockTestCase<SEED>("00000000000000000000000000000000", 16).Test("000102030405060708090a0b0c0d0e0f",
			"5ebac6e0054e166819aff1cc6d346cdb");
	res <<= 2;
	res |= BlockTestCase<SEED>("000102030405060708090a0b0c0d0e0f", 16).Test("00000000000000000000000000000000",
			"c11f22f20140505084483597e4370f43");
	res <<= 2;
	res |= BlockTestCase<SEED>("4706480851e61be85d74bfb3fd956185", 16).Test("83a2f8a288641fb9a4e9a5cc2f131c7d",
			"ee54d13ebcae706d226bc3142cd40d4a");
	res <<= 2;
	res |= BlockTestCase<SEED>("28dbc3bc49ffd87dcfa509b11d422be7", 16).Test("b41e6be2eba84a148e2eed84593c5ec7",
			"9b9b7bfcd1813cb95d0b3618f40f5122");

	return res;
}

	PLUGIN_STRUCTURE(seed, SEED)
	PLUGIN_DATA_START()
	PLUGIN_DATA(seed, "SEED")
	PLUGIN_DATA_END()
	PLUGIN_INTERFACE(seed)
}

typedef drew::SEED::endian_t E;

drew::SEED::SEED()
{
}

uint64_t drew::SEED::GenerateSubkey(uint32_t k[4], uint32_t kci)
{
	uint32_t ki0 = g(k[0] + k[2] - kci);
	uint32_t ki1 = g(k[1] - k[3] + kci);
	return (uint64_t(ki0) << 32) | ki1;
}

// The terms "odd" and "even" refer to RFC 4269, which uses 1-based indices.
uint64_t drew::SEED::OddKey(uint32_t k[4], uint32_t kci)
{
	uint64_t val = GenerateSubkey(k, kci);
	uint64_t sk = (uint64_t(k[0]) << 32) | k[1];
	sk = RotateRight(sk, 8);
	k[0] = uint32_t(sk >> 32);
	k[1] = uint32_t(sk);
	return val;
...
uint64_t drew::SEED::EvenKey(uint32_t k[4], uint32_t kci)
{
	uint64_t val = GenerateSubkey(k, kci);
	uint64_t sk = (uint64_t(k[2]) << 32) | k[3];
	sk = RotateLeft(sk, 8);
	k[2] = uint32_t(sk >> 32);
	k[3] = uint32_t(sk);
	return val;
...
int drew::SEED::SetKeyInternal(const uint8_t *key, size_t len)
{
	...
	m_k[ 0] =  OddKey(k, 0x9e3779b9);
	m_k[ 1] = EvenKey(k, 0x3c6ef373);
	m_k[ 2] =  OddKey(k, 0x78dde6e6);
	m_k[ 3] = EvenKey(k, 0xf1bbcdcc);
	m_k[ 4] =  OddKey(k, 0xe3779b99);
	m_k[ 5] = EvenKey(k, 0xc6ef3733);
	m_k[ 6] =  OddKey(k, 0x8dde6e67);
	m_k[ 7] = EvenKey(k, 0x1bbcdccf);
	m_k[ 8] =  OddKey(k, 0x3779b99e);
	m_k[ 9] = EvenKey(k, 0x6ef3733c);
	m_k[10] =  OddKey(k, 0xdde6e678);
	m_k[11] = EvenKey(k, 0xbbcdccf1);
	m_k[12] =  OddKey(k, 0x779b99e3);
	m_k[13] = EvenKey(k, 0xef3733c6);
	m_k[14] =  OddKey(k, 0xde6e678d);
	m_k[15] = EvenKey(k, 0xbcdccf1b);

	return 0;
}

uint64_t drew::SEED::f(uint64_t k, uint64_t r)
{
...
uint32_t drew::SEED::g(uint32_t x)
{
	return ss0[E::GetByte(x, 0)] ^ ss1[E::GetByte(x, 1)] ^
		ss2[E::GetByte(x, 2)] ^ ss3[E::GetByte(x, 3)];
}

int drew::SEED::Encrypt(uint8_t *out, const uint8_t *in) const
{
	uint64_t l, r;

	E::Copy(&l, in+0, sizeof(l));
	E::Copy(&r, in+8, sizeof(r));
	...
	E::Copy(out+0, &l, sizeof(l));
	E::Copy(out+8, &r, sizeof(r));
	return 0;
}

int drew::SEED::Decrypt(uint8_t *out, const uint8_t *in) const
{
	uint64_t l, r;
