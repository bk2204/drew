...
static int twofish128_test(void)
{
...
uint32_t drew::Twofish::Mod(uint32_t c) const
{
	...
	uint32_t c1 = c2 ^ (c >> 1) ^ ((c & 1) ? (mod >> 1) : 0);
...
uint32_t drew::Twofish::h0(uint32_t x, const uint32_t *k, size_t len) const
{
	...
	switch (len) {
		case 4:
			x = Q(q1, q0, q0, q1, x) ^ k[6];
		case 3:
			x = Q(q1, q1, q0, q0, x) ^ k[4];
		case 2:
			x = Q(q0, q1, q0, q1, x) ^ k[2];
			x = Q(q0, q0, q1, q1, x) ^ k[0];
	}
	return x;
}
...
const uint32_t drew::Twofish::mds[4][256] = {
{
